...
> 大多数语句（逻辑行）都包含了_表达式（Expressions）_。一个表达式的简单例子便是 `2+3`。表达式可以拆分成可以细分为运算符（Operators）与操作数（Operands）。
...
## 数值运算与赋值 / math operation and assignment
...
## 改变运算顺序 / Changing the Order Of Evaluation 

为了使表达式更加易读，我们可以使用括号。举个例子，`2 + (3 * 4)` 自是要比 `2 + 3 * 4` 要更加容易理解，因为后者还要求你要了解运算符的优先级。

使用括号还有一个额外的优点——它能帮助我们改变运算的顺序。举个例子: `(2 + 3) * 4`。
...
[123](file:///Users/yl/Documents/HUB/HTMLCSSJS_Study/Python/Python Study Notes/basics_zh.md)

[123](file:////basics_zh.md)
